...
class Data_Prepare(object):
    ...
    def __init__(self):
        self.concepts_lang = None  # 概念类
        self.diagnosis_lang = None  # 诊断类
        self.features_lang = None  # 特征类
    ...
    def get_concept_diagnosis_fathers_fetures(self, file_path:str):
        ...
        all_concept_fathers_diagnosis_features = []  # 重新组织的后的数据
        ...
        for line in new_data_content_2:
            ...
            a_concept_fathers_diagnosis_features = [concept_fathers, diagnosis, features]
            all_concept_fathers_diagnosis_features.append(a_concept_fathers_diagnosis_features)
        ...
        return new_data_content_2, all_concept_fathers_diagnosis_features, \
    ...
    def word_2_tensor(self, conceptFathers, diagnosis, features):
        ...
        if param.use_gpu:
            ...
            fathers_variable = [Variable(torch.LongTensor(i)).cuda() if i else None
                               for i in fathers_padded]
        else:
            fathers_variable = [Variable(torch.LongTensor(i)) if i else None
                                for i in fathers_padded]
    ...
    #         for s in fathers:
    #             for index in s:
    #                 if index != param.EOS_token:
    #                     temp.append(index)
    #         if len(temp) != 0:
    #             temp.append(param.EOS_token)
    #
    #         fathers_sequence.append(temp)
    #
    #     seq_pairs = sorted(zip(concept_sequence, diagnosis_sequence, features_sequence, fathers_sequence),
    #                        key=lambda p: len(p[0]), reverse=True)
    #     concept_sequence, diagnosis_sequence, features_sequence, fathers_sequence = zip(*seq_pairs)
    ...
    #                       if s else 0 for s in fathers_sequence]
    #
    #     # fathers_lengths = [len(s) for s in fathers_sequence]
    #     # fathers_padded = [self.pad_seq(s, max(fathers_lengths)) for s in fathers_sequence]
    #
    #     diagnosis_variable = Variable(torch.LongTensor(diagnosis_padded)).transpose(0, 1)
    #     concept_variable = Variable(torch.LongTensor(concept_padded)).transpose(0, 1)
    #     features_variable = Variable(torch.LongTensor(features_padded)).transpose(0, 1)
    #     # fathers_variable = Variable(torch.LongTensor(fathers_padded)).transpose(0, 1)
    #
    #     if param.use_gpu:
    #         diagnosis_variable = diagnosis_variable.cuda()
    #         concept_variable = concept_variable.cuda()
    #         features_variable = features_variable.cuda()
    #         fathers_variable = fathers_variable.cuda()
    #
    #     return concept_variable, concept_lengths, diagnosis_variable, diagnosis_lengths,\
    #            features_variable, features_lengths, fathers_variable, fathers_lengths
